execute unless block ~ ~ ~ #apolloparrot:non_solid run function apolloparrot:lunar-path/hit_block
execute if score #hit Lunar_Path matches 0 as @e[tag=!chainray,distance=..1.5] at @s run function apolloparrot:lunar-path/hit_entity
scoreboard players add #distance Lunar_Path 1
execute if score #hit Lunar_Path matches 0 if score #distance Lunar_Path matches ..400 positioned ^ ^ ^0.1 run function apolloparrot:lunar-path/ray_explode
