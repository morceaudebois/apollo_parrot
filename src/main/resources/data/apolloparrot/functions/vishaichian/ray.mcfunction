execute unless block ~ ~ ~ #apolloparrot:non_solid run function apolloparrot:vishaichian/hit_block
execute if score #hit Vishaichian matches 0 as @e[tag=!chainray,distance=..1.5] at @s run function apolloparrot:vishaichian/hit_entity
scoreboard players add #distance Vishaichian 1
execute if score #hit Vishaichian matches 0 if score #distance Vishaichian matches ..75 positioned ^ ^ ^0.1 run function apolloparrot:vishaichian/ray
